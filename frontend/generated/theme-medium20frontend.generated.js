...
import stylesCss from 'themes/medium20frontend/styles.css';
...
window.Vaadin['_vaadintheme_medium20frontend_globalCss'] = window.Vaadin['_vaadintheme_medium20frontend_globalCss'] || [];
export const applyTheme = (target) => {
  
  const injectGlobal = (window.Vaadin['_vaadintheme_medium20frontend_globalCss'].length === 0) || (!window.Vaadin['_vaadintheme_medium20frontend_globalCss'].includes(target) && target !== document);
  if (injectGlobal) {
    injectGlobalCss(stylesCss.toString(), target);
    
    window.Vaadin['_vaadintheme_medium20frontend_globalCss'].push(target);
  }
  if (!document['_vaadintheme_medium20frontend_componentCss']) {
    
    document['_vaadintheme_medium20frontend_componentCss'] = true;
  }
